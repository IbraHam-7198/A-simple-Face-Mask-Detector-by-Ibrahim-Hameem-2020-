...
The algorithm does a decent job in identifying if an individual is wearing a mask or not wearing a mask. However in the scenario that an individual is wearing a mask incorreclty the performance of the algorithm can be affected, depending on the type of mask and the location of the incorrectly worn mask. If the mask is right under the nose, the algorithm will start classifying some frames as "Mask" and the others as "No Mask or Incorrectly Masked". If the mask is about 2 cm below the nose then in most cases the algorithm is able to pick up that the mask is worn incorrectly. 

This issue primarily arises from the fact that there are a very few images in the training set that are wearing the mask incorrectly and the mask lies right below the nose. Most incorrectly masked images in the training set have the mask located right above the mouth (lips) but at least 2-3 cms below the nose. 
